        ...
        if (!GetComponent<Entity>().dead){
            ...
            GetComponent<Entity>().HitBox.GetComponent<HitBoxController>().currentHittingOpponent != null && !playingAttackAnim && !GetComponent<Entity>().burning){
                ...
                if (randomAttackAnim == 0){
                    gameObject.GetComponent<Entity>().animator.SetBool("DoubleSidedRavager_Attack_1",true);
                }
                else if (randomAttackAnim == 1){
                    gameObject.GetComponent<Entity>().animator.SetBool("DoubleSidedRavager_Attack_2",true);
                }
            }
            else if (!GetComponent<Entity>().HitBox.GetComponent<HitBoxController>().colliding && !playingAttackAnim && !GetComponent<Entity>().burning && !GetComponent<Entity>().gettingDarkMagicEffect)){
                gameObject.GetComponent<Entity>().animator.SetBool("DoubleSidedRavager_Attack_1",false);
                gameObject.GetComponent<Entity>().animator.SetBool("DoubleSidedRavager_Attack_2",false);
