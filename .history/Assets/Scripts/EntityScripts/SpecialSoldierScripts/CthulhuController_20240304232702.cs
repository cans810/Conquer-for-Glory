...
public class CthulhuController : MonoBehaviour
{
    ...
    { 
        if (!entity.dead){
            ...
            else if(!entity.HitBox.GetComponent<HitBoxController>().colliding && !rangedHitBox.GetComponent<HitBoxController>().colliding && !playingAttackAnim && canUseDarkMagic && darkMagicCount > 0 && foundEnemy != null && !foundEnemy.GetComponent<Entity>().dead){
                entity.animator.SetBool("Walk",false);
    ...
    private IEnumerator ResetSummonTimer()
    {
        ...
        if (darkMagicCount > 0 && searchForEnemys() != null && !foundEnemy.GetComponent<Entity>().dead && !entity.HitBox.GetComponent<HitBoxController>().colliding){
