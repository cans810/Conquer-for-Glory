...
public class OrcBeastController : MonoBehaviour
{
    ...
    void Update()
    {
        if (!entity.dead){
            ...
            && !playingAttackAnim && (float)entity.HitBox.GetComponent<HitBoxController>().currentHittingOpponent.entity.HP - (float)entity.damage > 0.0f){
            ...
            && !playingAttackAnim && (float)entity.HitBox.GetComponent<HitBoxController>().currentHittingOpponent.entity.HP - (float)entity.damage <= 0.0f
            && entity.HitBox.GetComponent<HitBoxController>().currentHittingOpponent.entity.canBeRipped){
                entity.animator.SetBool("OrcBeast_Walk",false);
            ...
            && !playingAttackAnim && (float)entity.HitBox.GetComponent<HitBoxController>().currentHittingOpponent.G.HP - (float)entity.damage <= 0.0f
            && !entity.HitBox.GetComponent<HitBoxController>().currentHittingOpponent.entity.canBeRipped){
                int randomAttack = UnityEngine.Random.Range(0,2);
