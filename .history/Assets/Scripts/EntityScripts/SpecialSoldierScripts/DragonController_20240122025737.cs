...
public class DragonController : MonoBehaviour
{
    ...
    void Update()
    {
        if (!GetComponent<Entity>().dead){
            ...
            else if (!GetComponent<Entity>().HitBox.GetComponent<HitBoxController>().colliding && !fireHitbox.GetComponent<DragonFireHitBoxController>().colliding && !playingAttackAnim){
    ...
    public void createFirePile(){
        GameObject firePile = Instantiate(firePilePrefab,transform.position,Quaternion.identity);
        firePile.transform.localScale = new Vector3(0.7f,0.7f,0.7f);
    }
}
