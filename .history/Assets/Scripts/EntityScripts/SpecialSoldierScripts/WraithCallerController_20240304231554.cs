...
public class WraithCallerController : MonoBehaviour
{
    ...
    { 
        if (!entity.dead){
            ...
            else if(!entity.HitBox.GetComponent<HitBoxController>().colliding && !rangedHitBox.GetComponent<HitBoxController>().colliding && !playingAttackAnim && canSummon && summonableSkeletonCount > 0 && foundEnemy != null && !foundEnemy.entity.dead){
                entity.animator.SetBool("Walk",false);
    ...
    public void summonSkeleton(){
        ...
        if (gameObject.tag.Equals("Player")){
            ...
            skeleton.entity.direction = "right";
            skeleton.entity.spawnedAtRow = foundEnemy.GetComponent<Entity>().spawnedAtRow;
        }
        else if (gameObject.tag.Equals("Enemy")){
            offset = new Vector3(+4f, -0.12f, 0f);
            skeleton.tag = "Enemy";
            skeleton.GetComponent<EntityCommonActions>().ChangeDirection("left");
            skeleton.entity.direction = "left";
            skeleton.entity.spawnedAtRow = foundEnemy.entity.spawnedAtRow;
        }
    ...
    private IEnumerator ResetSummonTimer()
    {
        ...
        if (summonableSkeletonCount > 0 && searchForEnemys() != null && !foundEnemy.entity.dead){
            canSummon = true;
        }
        else{
            canSummon = false;
