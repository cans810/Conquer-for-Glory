...
public class DragonController : MonoBehaviour
{
    ...
    void Update()
    {
        if (!GetComponent<Entity>().dead){
            // melee attack
            if (GetComponent<Entity>().HitBox.GetComponent<HitBoxController>().colliding && GetComponent<Entity>().HitBox.GetComponent<HitBoxController>().currentHittingOpponent != null && !playingMeleeAttackAnim && !playingFireAttackAnim){
        ...
        if (fireHitbox.GetComponent<DragonFireHitBoxController>().currentHittingOpponents.Count > 0 && fireHitbox.GetComponent<DragonFireHitBoxController>().currentHittingOpponents != null)
        {
            foreach (GameObject enemy in fireHitbox.GetComponent<DragonFireHitBoxController>().currentHittingOpponents){
                ...
                if (opponentEntity != null)
                {
                    ...
                    enemy.gameObject.GetComponent<Entity>().burning = true;
                    enemy.gameObject.transform.Find("SoundManager").GetComponent<EntitySoundManager>().playBurningScreamSound(8);
                    enemy.gameObject.GetComponent<Entity>().animator.SetBool("Burn",true);
                }
            }
        }
    }
