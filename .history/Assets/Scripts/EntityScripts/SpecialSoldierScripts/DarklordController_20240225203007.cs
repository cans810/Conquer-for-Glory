...
public class DarklordController : MonoBehaviour
{
    ...
    void Update()
    {
        if (!GetComponent<Entity>().dead){
            ...
            else if(!GetComponent<Entity>().HitBox.GetComponent<HitBoxController>().colliding && !rangedHitbox.GetComponent<HitBoxController>().colliding && !playingAttackAnim && canTeleport && teleportCount > 0 && foundEnemy != null && !foundEnemy.GetComponent<Entity>().dead
            && !GetComponent<Entity>().gettingDarkMagicEffect){
                gameObject.GetComponent<Entity>().animator.SetBool("Walk",false);
            ...
            else if (!GetComponent<Entity>().HitBox.GetComponent<HitBoxController>().colliding && !playingAttackAnim && !GetComponent<Entity>().gettingDarkMagicEffect){
