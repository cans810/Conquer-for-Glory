...
public class DarklordController : MonoBehaviour
{
    ...
    void Update()
    {
        if (!entity.dead){
            ...
            else if(!entity.HitBox.GetComponent<HitBoxController>().colliding && !rangedHitbox.GetComponent<HitBoxController>().colliding && !playingAttackAnim && canTeleport && teleportCount > 0 && foundEnemy != null && !foundEnemy.GetComponent<Entity>().dead
            && !entity.gettingDarkMagicEffect){
                entity.animator.SetBool("Walk",false);
