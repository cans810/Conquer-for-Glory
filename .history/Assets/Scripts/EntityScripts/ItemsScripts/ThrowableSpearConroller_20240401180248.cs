...
public class ThrowableSpearController : MonoBehaviour
{
    ...
    void Start()
    {
        if (sourceEntity.GetComponent<Entity>().soldierType == "Warlord"){
            spearDamage = 3.7f;
            transform.localScale = new Vector3(0.17f+0.010f,0.24f+0.010f,0.17f+0.010f);
        }
        else if (sourceEntity.GetComponent<Entity>().soldierType == "SpearThrower"){
            spearDamage = 1.8f;
            transform.localScale = new Vector3(0.15f+0.010f,0.15f+0.010f,0.15f+0.010f);
        }
    ...
    private void OnTriggerEnter2D(Collider2D collision){
        if (sourceEntity.tag.Equals("Player")){
            if (collision.CompareTag("Enemy") && spawnedAtRow==collision.gameObject.GetComponent<Entity>().spawnedAtRow)
            {
                ...
                && !collision.gameObject.GetComponent<Entity>().soldierType.Equals("Dragon") && !collision.gameObject.GetComponent<Entity>().soldierType.Equals("Mammoth")){
