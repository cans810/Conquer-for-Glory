    ...
    public void detachHead()
    {
        ...
        if (foundHead != null)
        {
            ...
            if (originalRenderer != null)
            {
                ...
                float randomLaunchForce = Random.Range(1, 4);

                Vector2 launchDirection = Quaternion.Euler(0, 0, randomAngle) * Vector2.up;
                Rigidbody2D rb = detachedHead.AddComponent<Rigidbody2D>();

                // Adjust Rigidbody2D properties for a lighter feel
                rb.mass = 0.5f; // Decrease mass
                rb.drag = 0.5f; // Apply some drag for slower movement
