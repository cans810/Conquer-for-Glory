...
public class RaceSelectionController : MonoBehaviour
{
    ...
    public List<GameObject> RaceModels;
    ...
    void Update()
    {
        currentSoldierShowcase = RaceModels[currentRace];

        foreach (GameObject soldier in RaceModels){
            if (soldier != currentSoldierShowcase){
        ...
        if (GameManager.Instance.finishedGameCtr >= 2 && !addedSeaElfs){
            racesNames.Add("SeaElf");
            addedSeaElfs = true;
        }
        else if (GameManager.Instance.finishedGameCtr < 2 && addedSeaElfs){
            racesNames.Remove("SeaElf");
            addedSeaElfs = false;
        }
    }
