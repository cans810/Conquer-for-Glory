...
public class RaceSelectionController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (currentRaceSelectedText.GetComponent<TextMeshProUGUI>().text.Equals("Demon") && GameManager.Instance.finishedGameCtr < 1){
            LockObject.SetActive(true);
            SetSpriteRenderersBlackRecursive(currentSoldierShowcase,Color.black);
        }
        else if (currentRaceSelectedText.GetComponent<TextMeshProUGUI>().text.Equals("Wraith") && GameManager.Instance.finishedGameCtr < 2){
            LockObject.SetActive(true);
            SetSpriteRenderersBlackRecursive(currentSoldierShowcase,Color.black);
        }
        else{
    ...
    {
        // Check if the current GameObject has a SpriteRenderer and is not named "Lock"
        SpriteRenderer spriteRenderer = obj.GetComponent<SpriteRenderer>();
        if (spriteRenderer != null && !obj.name.Equals("Lock"))
        {
            // Set the color of the SpriteRenderer to black
        ...
        {
            // Call the function recursively for each child GameObject
            SetSpriteRenderersBlackRecursive(child.gameObject);
        }
    }
