    ...
    void Start()
    {
        if (isConqueredByPlayer){
            ...
            if (canAttack) {
                ...
                attackablePointer.transform.localScale = new Vector3(0.6f, 0.6f, 0.6f); // Base size
                ...
                foreach (GameObject neighbour in Neighbours) {
                    if (neighbour != null && neighbour.GetComponent<CityInfo>().isConqueredByPlayer) {
                        ...
                        Vector3 midPoint = gameObject.transform.position + (direction.normalized * (distance / 2f));
                        attackablePointer.transform.position = midPoint; // Move to the midpoint
