            ...
            if (canAttack) {
                ...
                foreach (GameObject neighbour in Neighbours) {
                    if (neighbour != null && neighbour.GetComponent<CityInfo>().isConqueredByPlayer) {
                        ...
                        popupInfo.localRotation = Quaternion.Euler(0,0,0);
                        popupInfo.rotation = Quaternion.Euler(0,0,0);
                        popupInfo.localScale = new Vector3(1, 1, 1);
                        Quaternion reverseRotation = Quaternion.AngleAxis(0, Vector3.forward);
                        popupInfo.rotation = reverseRotation;
                    }
                }
            }
