    ...
    void Start()
    {
        if (isConqueredByPlayer){
            ...
            if (canAttack) {
                ...
                GameObject imageObject = attackablePointer.transform.Find("AttackablePointerImage").gameObject;
                imageObject.GetComponent<AttackablePointerController>().parentLand = attackablePointer.transform.parent.gameObject;
                
                foreach (GameObject neighbour in Neighbours) {
                    if (neighbour != null && neighbour.GetComponent<CityInfo>().isConqueredByPlayer && neighbour) {

                        Vector3 direction = neighbour.transform.position - gameObject.transform.position;
                        float distance = direction.magnitude;

                        imageObject.transform.localScale = new Vector3(distance*20, 100f, 100f);
                        ...
                        imageObject.GetComponent<AttackablePointerController>().difficulty = difficulty;
                    }
                }
            }
