    ...
    void Start()
    {
        if (isConqueredByPlayer){
            ...
            if (canAttack){
                ...
                attackablePointer.transform.localScale = new Vector3(0.6f, 0.6f, 0.6f);

                foreach (GameObject neighbour in Neighbours){
                    if (neighbour.GetComponent<CityInfo>().isConqueredByPlayer){
                        if (neighbour != null){
                            Vector3 direction = neighbour.transform.position - attackablePointer.transform.position;
                            attackablePointer.transform.rotation = Quaternion.LookRotation(direction);
                        }
                    }
                }
