            ...
            if (canAttack) {
                ...
                foreach (GameObject neighbour in Neighbours) {
                    if (neighbour != null && neighbour.GetComponent<CityInfo>().isConqueredByPlayer) {
                        RectTransform popupInfoRect = attackablePointer.transform.Find("PopupInfo").GetComponent<RectTransform>();

                        popupInfoRect.localEulerAngles = Vector3.zero;
                        popupInfoRect.localScale = Vector3.one;
                        popupInfoRect.anchoredPosition = Vector2.zero;
                        popupInfoRect.sizeDelta = Vector2.zero;
                        popupInfoRect.pivot = new Vector2(0.5f, 0.5f);
                        popupInfoRect.rotation = Quaternion.Euler(0,0,0);
                        ...
                        popupInfoRect = attackablePointer.transform.Find("PopupInfo").GetComponent<RectTransform>();
