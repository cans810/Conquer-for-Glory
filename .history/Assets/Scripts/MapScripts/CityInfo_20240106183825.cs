            ...
            if (canAttack) {
                ...
                foreach (GameObject neighbour in Neighbours) {
                    if (neighbour != null && neighbour.GetComponent<CityInfo>().isConqueredByPlayer) {
                        ...
                        Transform attackablePointerTransform = attackablePointer.transform;
                        Transform popupInfoTransform = attackablePointerTransform.Find("PopupInfo");

                        // Reset the scale to (1, 1, 1)
                        popupInfoTransform.localScale = Vector3.one;

                        // Reset the rotation to (0, 0, 90) regardless of the parent's rotation
                        Quaternion desiredRotation = Quaternion.Euler(0, 0, 90);
                        Quaternion inverseParentRotation = Quaternion.Inverse(attackablePointerTransform.rotation);
                        Quaternion finalRotation = desiredRotation * inverseParentRotation;

                        // Set the desired rotation
                        popupInfoTransform.rotation = finalRotation;
                        popupInfoTransform.localRotation = finalRotation;
                    }
                }
            }
