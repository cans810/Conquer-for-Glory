...
public class MapManager : MonoBehaviour
{
    ...
    void Update()
    {
        if (SceneManager.GetActiveScene().name.Equals("MapScene")){
            ...
            if(isEveryLandConquered() && !checkGameEnded){
                ...
                if (GameManager.Instance.finishedGameCtr == 2){
            ...
            if (GameManager.Instance.CurrentEnemyName && GameManager.Instance.CurrentEnemyRace && GameManager.Instance.CurrentEnemySoldiers)
        }
    }
    ...
    public void viewEast(){
        if (!viewingEast){
            MapObject = GameObject.Find("Map");
            viewingEast = true;
            MapObject.GetComponent<Animator>().SetTrigger("viewEast");
        }
    }
    ...
    public void viewWest(){
        if (viewingEast){
            MapObject = GameObject.Find("Map");
            viewingEast = false;
            MapObject.GetComponent<Animator>().SetTrigger("viewWest");
        }
    }
}
