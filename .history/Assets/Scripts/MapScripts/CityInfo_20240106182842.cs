            ...
            if (canAttack) {
                ...
                foreach (GameObject neighbour in Neighbours) {
                    if (neighbour != null && neighbour.GetComponent<CityInfo>().isConqueredByPlayer) {
                        ...
                        RectTransform attackablePointerTransform = attackablePointer.GetComponent<RectTransform>();
                        RectTransform popupInfoTransform = attackablePointer.transform.Find("PopupInfo").GetComponent<RectTransform>();

                        popupInfoTransform.SetParent(null);

                        popupInfoTransform.localRotation = Quaternion.identity;
                        popupInfoTransform.localScale = Vector3.one;
                        popupInfoTransform.SetParent(attackablePointerTransform);
                        popupInfoTransform.localScale = Vector3.one;
                    }
                }
            }
