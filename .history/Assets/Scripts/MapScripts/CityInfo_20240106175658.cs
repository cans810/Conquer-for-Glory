            ...
            if (canAttack) {
                ...
                foreach (GameObject neighbour in Neighbours) {
                    if (neighbour != null && neighbour.GetComponent<CityInfo>().isConqueredByPlayer) {
                        ...
                        Transform popupInfo = attackablePointer.transform.Find("PopupInfo");
            Vector3 originalPosition = popupInfo.position; // Store the original position
            popupInfo.localPosition = Vector3.zero; // Reset local position
            popupInfo.localRotation = Quaternion.identity; // Reset local rotation
            popupInfo.localScale = Vector3.one; // Reset local scale
            popupInfo.position = originalPosition; // Restore original world position
                    }
                }
            }
