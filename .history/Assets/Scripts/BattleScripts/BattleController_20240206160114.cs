...
public class BattleController : MonoBehaviour
{
    ...
    void Update()
    {
        if (!battleEnded){
            ...
            if (Input.GetKeyDown(KeyCode.Space) || SettingsManager.Instance.autoSummon && enemyDeathCounterUlti < 20)
            {
                Transform parentTransform = PlayerSoldierContainers.transform;
                ...
                if (parentTransform.GetChild(currentSelectedSoldierContainer).GetComponent<SoldierContainerManager>().canSummon){
                    ...
                    Vector3 spawnPosition = summonPoint.position - new Vector3(0.8f, summonPointHeight / 1.8f, 0);
                ...
                if (Input.GetKeyDown(KeyCode.Q))
                {
                    ...
                    if (parentTransform.GetChild(currentSelectedSoldierContainer).GetComponent<SoldierContainerManager>().canSummon){
                        ...
                        for (int i=0;i<8;i++){
                            ...
                            Vector3 spawnPosition = summonPoint.position - new Vector3(0.8f, summonPointHeight / 2.2f, 0);
