...
public class BattleController : MonoBehaviour
{
    ...
    void Update()
    {
        if (!battleEnded){
            ...
            if (SettingsManager.Instance.autoSummon && enemyDeathCounterUlti < 18 && !SettingsManager.Instance.battlePaused)
            {
    ...
    public void pressedRIGHT(){
        OnButtonPressed(RightButton.GetComponent<Button>());
    ...
    public void pressedLEFT(){
        OnButtonPressed(LeftButton.GetComponent<Button>());
    ...
    public void pressedUP(){
        OnButtonPressed(UpButton.GetComponent<Button>());
        if (!battleEnded){
            if (!SettingsManager.Instance.battlePaused)
            {
    ...
    public void pressedDOWN(){
        OnButtonPressed(DownButton.GetComponent<Button>());
        if (!battleEnded){
            if (!SettingsManager.Instance.battlePaused)
            {
                if (currentSelectedSummonPoint+1 <= 6){
                    Transform parentTransform = PlayerSummonPoints.transform;
                    parentTransform.GetChild(currentSelectedSummonPoint).GetComponent<SummonPointManager>().selected = false;
    ...
    public void pressedSUMMON(){
        OnButtonPressed(SummonButton.GetComponent<Button>());

        if (!battleEnded){
            if (enemyDeathCounterUlti < 18 && !SettingsManager.Instance.battlePaused)
            {
                Transform parentTransform = PlayerSoldierContainers.transform;
    ...
    {
        button.image.color = new Color(button.image.color.r,button.image.color.g,button.image.color.b,162f/255f);

        StartCoroutine(ResetButtonColorCoroutine(button));
    }

    private IEnumerator ResetButtonColorCoroutine(Button button)
    {
        yield return new WaitForSeconds(0.1f);

        button.image.color = button.image.color = new Color(button.image.color.r,button.image.color.g,button.image.color.b,72f/255f);
    }
