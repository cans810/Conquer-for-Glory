...
public class BattleGroundSoundsManager : MonoBehaviour
{
    ...
    private IEnumerator PlayAndFade(AudioSource source)
    {
        ...
        {
            source.volume = Mathf.Lerp(0f, SettingsManager.Instance.BattleSoundsVolume, timer / fadeInDuration);
        ...
        {
            source.volume = Mathf.Lerp(SettingsManager.Instance.BattleSoundsVolume, 0f, timer / fadeOutDuration);
            timer += Time.deltaTime;
            yield return null;
        }

        source.Stop();
        source.volume = SettingsManager.Instance.BattleSoundsVolume;
    }
