...
public class EnemySummonController : MonoBehaviour
{
    ...
    public float dynamicRowSelectionModifier;

    public void Awake(){
        dynamicRowSelectionModifier = 0;
    ...
    void Update()
    {
            ...
            if (parentTransformSoldierContainers != null){
                ...
                if (parentTransformSoldierContainers.GetChild(currentSelectedSoldierContainer).GetComponent<SoldierContainerManager>().canSummon && battlecontroller.GetComponent<BattleController>().playerDeathCounterUlti >= 18){
                ...
                else if (parentTransformSoldierContainers.GetChild(currentSelectedSoldierContainer).GetComponent<SoldierContainerManager>().canSummon && battlecontroller.GetComponent<BattleController>().playerDeathCounterUlti < 18){
                    summonPointSelector();
    ...
    public void summonPointSelector(){
        ...
        // Check if all rows have the same number of soldiers
        bool sameSoldiersInAllRows = true;
        int firstRowCount = rowsCtrDict["0"];
        foreach (var row in rowsCtrDict)
        {
            if (row.Value != firstRowCount)
            {
                sameSoldiersInAllRows = false;
                break;
            }
        }

        // If all rows have the same number of soldiers, select a random row
        if (sameSoldiersInAllRows)
        {
            currentSelectedSummonPoint = UnityEngine.Random.Range(0, 7);
        }
        else{
            int highestValue = -1;
            int highestKey = -1;
            ...
            for (int i=0; i<rowsCtrDict.Count; i++){
                if (rowsCtrDict[i.ToString()] > highestValue)
                {
                    highestValue = rowsCtrDict[i.ToString()];
                    highestKey = i;
                }
            }

            dynamicRowSelectionModifier = highestValue*5f;
            
            float randomDecision = Random.Range(0,100);

            if (randomDecision <= (35 + dynamicRowSelectionModifier)){

                if (players == null || players.Length == 0){
                    int randomPoint = UnityEngine.Random.Range(0, 7);
                    currentSelectedSummonPoint = randomPoint;
                }
                else{
                    currentSelectedSummonPoint = highestKey;
                }
